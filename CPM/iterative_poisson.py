# -*- coding: utf-8 -*-
"""iterative-poisson.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1CUUdL3n98OYspi6P6sfImGrBRZS73ree
...
We construct the Laplacian and Interpolation matrices L and E as usual. Then we use Ruuth-Merriman Jacobi Iteration, which consists of repeatedly applying two steps:


1.   $u \leftarrow \mathrm{diag}(L)^{-1} (f - (L - \mathrm{diag}(L)) u ) $
2.   $u \leftarrow E u$

This is similar to the standard Jacobi iteration procedure except we apply the closest point extension matrix $E$ after each iteration. The reason we use an iterative procedure rather than simply inverting the matrix in this case is that the matrix $M = EL$ is actually singular. The iterative method can also be faster in cases with sparse matrices (which is what we have).
...
"""Note that $\Delta_s u = \frac{1}{r^2} u_{\theta \theta} + \frac{1}{r} u_r + u_{rr}$ (this tells us how to take the Laplacian of a function in terms of $r$ and $\theta$)."""
...
#u = np.array([init(all_pts[i]) for i in band])
